    ...
    for (int j = index; j < m; j += stride) {
      ...
      if((ingroup_pts_cnt[j]>=3)){
        ...
        for (int k=0;k<n;++k) {
          ...
          rspoint[0] = ((*(v+(c*c*j)+6)))*spoint[0]+((*(v+(c*c*j)+3)))*spoint[1]+((*(v+(c*c*j)+0)))*spoint[2];
          rspoint[1] = ((*(v+(c*c*j)+7)))*spoint[0]+((*(v+(c*c*j)+4)))*spoint[1]+((*(v+(c*c*j)+1)))*spoint[2];
          rspoint[2] = ((*(v+(c*c*j)+8)))*spoint[0]+((*(v+(c*c*j)+5)))*spoint[1]+((*(v+(c*c*j)+2)))*spoint[2];
          float xx = rspoint[0];
          float yy = rspoint[1];
          float zz = rspoint[2];
