...
class QueryAndGroupE(nn.Module):
    r"""
    Groups with a ball query of radius
    ...
    ---------
    e1, e2, e3 : float32
        e1, e2, e3 of ellipsoid
    nsample : int32
        Maximum number of points to gather in the ball
    """

    def __init__(self, e1: float, e2: float, e3: float, nsample: int, use_xyz: bool = True):
        super().__init__()
        self.e1, self.e2, self.e3, self.nsample, self.use_xyz = e1, e2, e3, nsample, use_xyz
    ...
    def forward(
        ...
        idx,d = ellipsoid_query(self.e1, self.e2, self.e3, self.nsample, xyz, new_xyz, fps_idx)#,ingroup_pts_cnt, ingroup_out, ingroup_cva, v, d)
        xyz_trans = xyz.transpose(1, 2).contiguous()
        d = d.transpose(1,2).unsqueeze(-1)
        grouped_xyz = grouping_operation(
            xyz_trans, idx
        )  # (B, 3, npoint, nsample)
        raw_grouped_xyz = grouped_xyz
        ...
        if features is not None:
            ...
            assert self.use_xyz, "Cannot have not features and not use xyz as a feature!"
            new_features = torch.cat([raw_grouped_xyz, grouped_xyz], dim = 1)
        # d = d.repeat([1,1,1,new_features.size(3)])
        # print(d.size())
        # print(new_features.size())
        # new_features = torch.cat([new_features, d],dim=1)
        # print(new_features.size())
